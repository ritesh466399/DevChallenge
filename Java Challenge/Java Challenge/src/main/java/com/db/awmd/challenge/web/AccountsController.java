...
public class AccountsController {
  ...
  public ResponseEntity<Object> transferBalance(@RequestBody @Valid Account accountFrom,@RequestBody @Valid Account accountTo,
		  @RequestBody @Valid BigDecimal amount) {
    log.info("balance taransfer from ", accountFrom.getAccountId() + "to " +accountTo.getAccountId());

    try {
    	this.accountsService.transfer(accountFrom,accountTo ,amount);
    } catch (InsufficientBalanceExcepion ibe) {
      return new ResponseEntity<>(ibe.getMessage(), HttpStatus.BAD_REQUEST);
    } catch (NegativeAmountExcepion nae) {
